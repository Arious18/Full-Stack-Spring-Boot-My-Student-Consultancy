import React, { useState, useEffect } from 'react';
import { useNavigate } from 'react-router-dom';
import { useTranslation } from 'react-i18next';
import axios from 'axios';
import Header from "../header/Header.jsx";
import Footer from "../footer/Footer.jsx";
import './style.css';

function FacultyList() {
    const [faculties, setFaculties] = useState([]);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState(null);
    const [universityMap, setUniversityMap] = useState({});
    const navigate = useNavigate();
    const { t } = useTranslation();

    useEffect(() => {
        const fetchData = async () => {
            try {
                setLoading(true);
                // Fetch all faculties with correct URL
                const facultyResponse = await axios.get('http://localhost:8080/faculties');
                console.log('Faculties data received:', facultyResponse.data);

                // Fetch all universities for mapping with correct URL
                const universityResponse = await axios.get('http://localhost:8080/universities');
                console.log('Universities data received:', universityResponse.data);

                const universityData = universityResponse.data.reduce((acc, university) => {
                    acc[university.id] = university;
                    return acc;
                }, {});

                setUniversityMap(universityData);

                // Sort faculties alphabetically by name
                const sortedFaculties = facultyResponse.data.sort((a, b) =>
                    a.name.localeCompare(b.name)
                );

                setFaculties(sortedFaculties);
                setError(null);
            } catch (err) {
                console.error('Error fetching data:', err);
                setError(t('errorFetchingData') || 'Failed to load faculties. Please try again.');
            } finally {
                setLoading(false);
            }
        };

        fetchData();
    }, [t]);

    const handleFacultyClick = (id) => {
        navigate(`/faculty/${id}`);
    };

    if (loading) return <div className="list-loading">{t('loading') || 'Loading...'}</div>;
    if (error) return <div className="list-error">{error}</div>;

    return (
        <div className="university-page-wrapper">
            <Header />
            <div className="list-container">
                <h2 className="list-title">{t('faculties') || 'Faculties'}</h2>
                <div className="list-wrapper">
                    {faculties.length === 0 ? (
                        <p className="list-empty">{t('noFacultiesFound') || 'No faculties found'}</p>
                    ) : (
                        faculties.map((faculty) => {
                            const university = universityMap[faculty.universityId] || {};
                            return (
                                <div
                                    key={faculty.id}
                                    className="list-item"
                                    onClick={() => handleFacultyClick(faculty.id)}
                                >
                                    <div className="list-item-content">
                                        <div className="list-item-image-container">
                                            {faculty.imageUrl ? (
                                                <img
                                                    src={faculty.imageUrl}
                                                    alt={faculty.name}
                                                    className="list-item-image"
                                                    onError={(e) => {
                                                        e.target.onerror = null;
                                                        e.target.src = 'https://placehold.co/600x400/e2e8f0/1e293b?text=Faculty';
                                                    }}
                                                />
                                            ) : (
                                                <div className="list-item-image-placeholder">
                                                    {faculty.name.charAt(0)}
                                                </div>
                                            )}
                                        </div>
                                        <div className="list-item-details">
                                            <h3 className="list-item-name">{faculty.name}</h3>
                                            <p className="list-item-university">
                                                {university.name || t('unknownUniversity') || 'Unknown University'}
                                            </p>
                                            <p className="list-item-country">
                                                {university.country ? university.country.name : (t('noCountry') || 'No Country')}
                                            </p>
                                            <div className="list-item-price-container">
                                                {faculty.price && (
                                                    <>
                                                        {faculty.discountPrice && faculty.discountPrice < faculty.price ? (
                                                            <>
                                                                <span className="list-item-price-original">${faculty.price.toLocaleString()}</span>
                                                                <span className="list-item-price-discount">${faculty.discountPrice.toLocaleString()}</span>
                                                            </>
                                                        ) : (
                                                            <span className="list-item-price">${faculty.price.toLocaleString()}</span>
                                                        )}
                                                    </>
                                                )}
                                            </div>
                                        </div>
                                    </div>

                                    {/* Action buttons */}
                                    <div className="list-item-actions">
                                        <button
                                            className="list-item-button secondary"
                                            onClick={(e) => {
                                                e.stopPropagation();
                                                navigate(`/fields/${faculty.id}`);
                                            }}
                                        >
                                            {t('viewFields') || 'View Fields'}
                                        </button>
                                    </div>
                                </div>
                            );
                        })
                    )}
                </div>
            </div>
            <Footer />
        </div>
    );
}

export default FacultyList;